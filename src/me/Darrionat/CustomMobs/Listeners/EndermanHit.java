...
public class EndermanHit implements Listener {
	private Main plugin;

	public EndermanHit(Main plugin) {
		this.plugin = plugin;
	...
	public void onHit(EntityDamageByEntityEvent e) {
		...
		int percent = config.getInt(entityType + ".levitationPercent");
		if (i > percent) {
			return;
		}
		((LivingEntity) e.getEntity()).addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION,  5 * 20, 1));
	}
}
